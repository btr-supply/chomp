...
def create_instance_monitor():
    ...
    instance_name = (
        state.instance.name
        if hasattr(state, "instance") and state.instance
        else "chomp_instance"
    )
...
async def start_ingester(config: Config):
    ...
    try:
        ...
        if not redis_connected:
            log_error("Failed to connect to Redis. Cannot proceed with task claiming.")
            return
    ...
    try:
        ...
        all_tasks = [
            task for result in scheduled_results for task in result if task is not None
        ]
        if all_tasks:
...
async def main(ap: ArgParser):
    ...
    if state.args.ping:
        ...
        return (
            0 if (db_ok and cache_ok) else 1
...
if __name__ == "__main__":
  ...
  \\__/_//_/\\___/_/_/_/ .__/
    ...
    ap = ArgParser(
        description="Chomp retrieves, transforms and archives data from various sources."
    )
    ap.add_groups(
        {
            "Common runtime": [
