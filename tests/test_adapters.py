...
def test_adapter_structure(adapter_path: str, expected_class: str) -> tuple[bool, str]:
    ...
    try:
        ...
        required_methods = [
            "connect", "ping", "ensure_connected", "close",
            "create_db", "use_db", "create_table", 
            "insert", "insert_many", "fetch", "fetchall", 
            "fetch_batch", "commit", "list_tables"
        ]
        
        methods = [node.name for node in adapter_class.body if isinstance(node, ast.FunctionDef)]
...
def main():
    ...
    for adapter_path, expected_class in adapters:
        ...
        # Test structure
        structure_ok, structure_msg = test_adapter_structure(adapter_path, expected_class)
        if structure_ok:
