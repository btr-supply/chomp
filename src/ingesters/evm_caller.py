...
async def schedule(c: Ingester) -> list[Task]:
  ...
  async def ingest(c: Ingester):
    ...
    for field in c.fields:
      if not field.target or field.id in unique_calls:
        if field.id in unique_calls:
          log_warn(
              f"Duplicate target smart contract view {field.target} in {c.name}.{field.name}, skipping..."
          )
        continue
    ...
    async def execute_multicall(m: Multicall,
      ...
      while not output and retry_count < max_retries:
        ...
        try:
          ...
          log_error(
              f"Multicall for chain {m.w3.eth.chain_id} failed: {e}, switching RPC..."
          )
          prev_rpc = m.w3.provider.endpoint_uri
          m.w3 = await state.web3.client(m.w3.eth.chain_id,
                                         roll=True)  # Keep as Any type for now
    ...
    async def call_multi(m: Multicall,
                         retry_batch_size=4,
      ...
      try:
        ...
        if len(m.calls) > max_batch_size:
          if state.args.verbose:
            log_debug(
                f"Calls exceed max_batch_size ({len(m.calls)} > {max_batch_size}), splitting into batches..."
            )
          all_results = {}

          for i in range(0, len(m.calls), max_batch_size):
            batch = m.calls[i:i + max_batch_size]
            batch_multi = Multicall(calls=batch,
                                    _w3=m.w3,
                                    require_success=True,
                                    gas_limit=mc_const.GAS_LIMIT)
            try:
              ...
              if batch_output:
                ...
                sub_results = await handle_failed_batch(
                    batch_multi, retry_batch_size)
                all_results.update(sub_results)
            except Exception as e:
              log_warn(
                  f"Batch {i//max_batch_size + 1} failed ({str(e)}), trying smaller batches..."
              )
              sub_results = await handle_failed_batch(batch_multi,
                                                      retry_batch_size)
              all_results.update(sub_results)
        ...
        if not split_on_failure:
          log_error(
              f"Multicall failed for chain {m.w3.eth.chain_id}, skipping...")
          return {}
        ...
        log_error(
            f"Failed to execute multicall for chain {m.w3.eth.chain_id}: {e}")
        return {}
    ...
    async def handle_failed_batch(m: Multicall, retry_batch_size: int) -> dict:
      ...
      log_warn(
          f"Multicall failed, splitting into batches of {retry_batch_size} and processing concurrently..."
      )
      all_results = {}
      ...
      for i in range(0, len(m.calls), retry_batch_size):
        ...
        try:
          ...
          log_warn(
              f"Batch {i + 1} failed ({str(e)}), trying individual calls...")

          for call in batch:
            single_multi = Multicall(calls=[call],
                                     _w3=m.w3,
                                     require_success=True,
                                     gas_limit=mc_const.GAS_LIMIT)
            try:
              ...
              log_error(
                  f"Individual call failed - Target: {call.target}, Function: {call.function[0]}, Error: {str(e)}"
              )
