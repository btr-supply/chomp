...
async def fetch_json(url: str,
                     ...
                     monitor: Optional['Monitor'] = None) -> str:
  """
  Fetch JSON data from URL
  Returns: response_text
  """
  if max_retries is None:
    max_retries = getattr(state.args, 'max_retries', 3) if hasattr(
        state, 'args') else 3
  ...
  async def ingest(c: Ingester) -> None:
    ...
    async def fetch_hashed(url: str) -> dict:
      try:
        ...
        data_str = await get_or_set_cache(hashes[url], fetch_data_only,
                                          c.interval_sec)
    ...
    for field in c.fields:
      if field.target:
        ...
        url = url.strip().format(
            **c.data_by_field)  # inject fields inside url if required
