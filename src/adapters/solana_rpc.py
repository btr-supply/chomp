...
class SolanaRpcClient(JsonRpcClient):
  ...
  async def get_balance(self, address: str) -> int:
    return await self.call("getBalance", [address, {"commitment": "finalized"}])

  async def get_account_info(self, address: str, encoding: str = "base64") -> Optional[dict]:
    result = await self.call("getAccountInfo", [address, {"encoding": encoding}])
    return result.get("value") if result else None

  async def get_token_balances(self, address: str) -> list[dict]:
    return await self.call("getTokenAccountsByOwner", [address, {"programId": "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA"}])

  async def get_signature_status(self, signature: str) -> Optional[dict]:
    result = await self.call("getSignatureStatuses", [[signature], {"searchTransactionHistory": True}])
  ...
  async def get_recent_blockhash(self) -> str:
    return (await self.call("getRecentBlockhash", {"commitment": "finalized"}))["blockhash"]
  ...
  async def get_multi_accounts(self, addresses: list[str], encoding: str = "base64") -> list[Optional[dict]]:
  ...
  async def verify_signature(self, signature: str) -> bool:
    try:
      ...
      return (
          status.get("confirmationStatus") == "finalized" and 
          not status.get("err")
      )
    except Exception:
      return False
  ...
  async def verify_signatures(self, signatures: list[str]) -> list[bool]:
    result = await self.call("getSignatureStatuses", [signatures, {"searchTransactionHistory": True}])
    statuses = result.get("value", []) if result else []
    return [
      bool(status and status.get("confirmationStatus") == "finalized" and not status.get("err"))
