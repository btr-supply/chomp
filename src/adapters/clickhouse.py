...
class ClickHouse(Tsdb):
  ...
  async def connect(
    ...
    self = cls(
      host=host or env.get("CLICKHOUSE_HOST") or "localhost",
  ...
  async def ensure_connected(self):
    if not self.conn:
      try:
        ...
        log_info(f"Connected to ClickHouse on {self.host}:{self.port}/{self.db} as {self.user}")
      except Exception as e:
        e_str = str(e).lower()
        if "database" in e_str and "doesn't exist" in e_str:
          ...
          raise ValueError(f"Failed to connect to ClickHouse on {self.user}@{self.host}:{self.port}/{self.db}")

    if not self.cursor:
      self.cursor = await self.conn.cursor()
    if not self.cursor:
      raise ValueError(f"Failed to create cursor for ClickHouse on {self.user}@{self.host}:{self.port}/{self.db}")
  ...
  async def create_db(self, name: str, options={}, force=False):
    ...
    for i in range(max_retries):
      try:
        ...
        log_warn(f"Retrying to create database {name} in {i+1}/{max_retries} ({e})...")
        await sleep(1)
    ...
    if i < max_retries - 1:
      ...
      for j in range(max_retries):
        try:
          ...
          log_warn(f"Retrying to use database {name} in {j+1}/{max_retries} ({e})...")
          await sleep(1)
  ...
  async def alter_table(self, table: str, add_columns: list[tuple[str, str]] = [], drop_columns: list[str] = []):
    ...
    for column_name, column_type in add_columns:
      try:
        await self.cursor.execute(f"ALTER TABLE {self.db}.`{table}` ADD COLUMN `{column_name}` {column_type}")
    ...
    for column_name in drop_columns:
      try:
        await self.cursor.execute(f"ALTER TABLE {self.db}.`{table}` DROP COLUMN `{column_name}`")
  ...
  async def insert_many(self, c: Ingester, values: list[tuple], table=""):
    ...
    placeholders = ", ".join(["%s" for _ in range(len(persistent_fields) + 1)])  # +1 for timestamp
  ...
  async def fetch(
    ...
    select_cols = [f"{'argMin' if use_first else 'argMax'}(`{col}`, ts) AS `{col}`" for col in columns if col != 'ts']
    select_cols.insert(0, f"toStartOfInterval(ts, INTERVAL {agg_seconds} second) AS ts")
