...
class SQLite(SqlAdapter):
  ...
  async def alter_table(self, table: str, add_columns: list[tuple[str, str]] = [], drop_columns: list[str] = []):
    ...
    if drop_columns:
      log_error(f"SQLite doesn't support dropping columns directly. Columns {drop_columns} will remain in table {table}")
  ...
  async def list_tables(self) -> list[str]:
    ...
    await self.ensure_connected()
    try:
      result = await self._fetch("SELECT name FROM sqlite_master WHERE type='table'")
