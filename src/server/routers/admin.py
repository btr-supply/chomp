...
@limit(points=5)
async def validate_config(req: Request,
  ...
  try:
    ...
    return ApiResponse({
        ...
        "affected_ingesters":
        validation_result.get("affected_ingesters", []),
    })
  except Exception as e:
...
async def update_config(req: Request,
  ...
  try:
    ...
    return ApiResponse({
        ...
        "affected_ingesters":
        update_result["affected_ingesters"],
        "restart_required":
        update_result.get("restart_required", []),
        "timestamp":
...
async def upload_config(
  ...
  try:
    ...
    if not validation_result["valid"]:
      raise HTTPException(status_code=400,
                          detail="Uploaded configuration is invalid")

    return ApiResponse({
        "success":
        True,
        "message":
        "Configuration file uploaded and validated successfully",
        "config":
        config_content,
        "affected_ingesters":
        validation_result.get("affected_ingesters", []),
...
async def rollback_config(req: Request, version_id: str) -> ApiResponse:
  ...
  try:
    ...
    return ApiResponse({
        ...
        "rolled_back_to":
        rollback_result["version_id"],
        "affected_ingesters":
        rollback_result["affected_ingesters"],
    })
  except Exception as e:
...
async def get_ingester_status(req: Request) -> ApiResponse:
  ...
  try:
    ingesters = await hse(admin_service.get_all_ingester_status()
    ...
    raise HTTPException(status_code=500,
                        detail="Failed to retrieve ingester status")
...
async def get_ingester_details(req: Request,
  ...
  try:
    details = await hse(admin_service.get_ingester_details(ingester_name)
                        )  # type: ignore[attr-defined]
...
async def control_ingesters(
  ...
  try:
    ...
    raise HTTPException(status_code=500,
                        detail="Ingester control operation failed")
...
async def restart_ingester(req: Request,
  ...
  try:
    result = await hse(admin_service.restart_ingester(ingester_name, force)
                       )  # type: ignore[attr-defined]
    return ApiResponse({
    ...
    raise HTTPException(status_code=500,
                        detail=f"Failed to restart ingester {ingester_name}")
...
async def get_system_logs(req: Request,
  ...
  try:
    logs = await hse(admin_service.get_system_logs(level, limit, since)
                     )  # type: ignore[attr-defined]
    return ApiResponse({
        "logs": logs,
        "count": len(logs),
...
async def get_database_status(req: Request) -> ApiResponse:
  ...
  try:
    status = await hse(admin_service.get_database_status()
                       )  # type: ignore[attr-defined]
    return ApiResponse(status)
  except Exception as e:
    log_error(f"Failed to get database status: {e}")
    raise HTTPException(status_code=500,
                        detail="Failed to retrieve database status")
...
async def get_database_tables(req: Request) -> ApiResponse:
  ...
  try:
    tables = await hse(admin_service.get_database_tables()
                       )  # type: ignore[attr-defined]
...
async def get_cache_status(req: Request) -> ApiResponse:
  ...
  try:
    status = await hse(admin_service.get_cache_status()
                       )  # type: ignore[attr-defined]
    return ApiResponse(status)
  except Exception as e:
...
async def clear_cache(req: Request,
  ...
  try:
    result = await hse(admin_service.clear_cache(pattern)
                       )  # type: ignore[attr-defined]
    return ApiResponse({
        "success": result["success"],
