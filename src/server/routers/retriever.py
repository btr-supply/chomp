...
async def ping(
  ...
  utc_time: int = Query(
    None,
    description="Client UTC timestamp in milliseconds"
  )
):
...
async def get_history(
  ...
  data = await hse(loader.get_history(
    parsed_resources, parsed_fields,
    from_date, to_date, interval, quote, precision, format
  ))
