...
TsdbAdapter = Literal["tdengine", "timescale", "influx", "kx", "sqlite",
                      "clickhouse", "duckdb", "mongodb", "opentsdb",
                      "victoriametrics"]
...
class Monitor:
  ...
  def stop_timer(self,
                 response_bytes: int,
                 status_code: Optional[int] = None) -> RequestVitals:
    """Stop timer and return metrics"""
    end_time = time.time()
    latency_ms = (end_time -
                  self._start_time) * 1000 if self._start_time > 0 else 0.0

    # Store vitals on the monitor for later collection by store()
    self.vitals = RequestVitals(latency_ms=latency_ms,
...
class Targettable:
  ...
  def target_id(self) -> str:
    return md5(
        (self.target + self.selector + str(self.params) + str(self.actions) +
         function_signature(self.handler)).encode()).hexdigest()
...
class ResourceField(Targettable):
  ...
  def sql_escape(self) -> str:
    return f"'{self.value}'" if self.type in ["string", "binary", "varbinary"
                                              ] else str(self.value)
  ...
  def to_dict(self, scope: Scope = Scope.DEFAULT) -> dict:
    return {
        ...
        "transformers":
        self.transformers if scope & Scope.TRANSFORMERS else None,
        "tags": self.tags,
...
class Resource:
  ...
  def field_by_name(self, include_transient=False) -> dict[str, ResourceField]:
    return {
        field.name: field
        for field in self.fields if not field.transient or include_transient
    }
...
class Ingester(Resource, Targettable):
  ...
  def from_config(cls, d: dict):
    ...
    if 'fields' in d and isinstance(d['fields'], list):
      d['fields'] = [
          ResourceField.from_dict(field) if isinstance(field, dict) else field
      ...
      if field_item.params:
        ...
        for i, param in enumerate(field_item.params):
          if isinstance(param, dict):
            ...
            if param.get("type") == "time" and not param.get(
                "value") and not param.get("default"):
              param["default"] = round(now().timestamp())
          if isinstance(
              field_item.params, dict
          ) and "time" in field_item.params:  # TODO: genericize to all time fields
  ...
  def values_dict(self):
    d = {
        ...
        for field in self.fields if not field.transient
    }
  ...
  def dependencies(self) -> list[str]:
    ...
    return list({
        ...
        for field in self.fields if field.selector and field.selector[0] != '.'
        and '.' in field.selector
    })
...
class ConfigMeta(type):
  ...
  def __new__(cls, name, bases, dct):
    ...
    for ingester_type in get_args(IngesterType):
      ...
      dct.setdefault(
          '__annotations__',
          {})[attr_name] = list[Ingester]  # add type hint to the class
...
class Config(metaclass=ConfigMeta):
  ...
  def from_dict(cls, data: dict) -> 'Config':
    ...
    for ingester_type in get_args(IngesterType):
      ...
      config_dict[key] = [
          Ingester.from_config({
              **item, 'ingester_type': ingester_type
          }) for item in items
      ]
...
class Tsdb:
  ...
  async def fetch(self, table: str, from_date: datetime, to_date: datetime,
                  aggregation_interval: Interval, columns: list[str]):
    raise NotImplementedError

  async def fetchall(self):
    raise NotImplementedError

  async def fetch_batch(self, tables: list[str], from_date: datetime,
                        to_date: datetime, aggregation_interval: Interval,
...
class Instance:
  ...
  async def get_ip_addresses(self) -> tuple[Optional[str], Optional[str]]:
    ...
    async def get_ip(url: str) -> Optional[str]:
      try:
        ...
        return response.json().get(
            "ip") if response.status_code == 200 else None
      except Exception:
        return None
    ...
    async with httpx.AsyncClient(timeout=3.0) as session:
      ...
      ip_results = await gather(get_ip("https://api4.ipify.org/?format=json"),
