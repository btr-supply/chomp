...
async def convert(pair: str, base_amount: float = None, quote_amount: float = None, precision: int = 6) -> ServiceResponse[dict]:
  """Convert between two resources using their last values"""
  # Validate pair format
  if not ('-' in pair and '.' in pair):
    return "Invalid pair format. Must be 'resource.field-resource.field'", None
  if base_amount and quote_amount:
    return "Cannot specify both base and quote amounts", None
  ...
  try:
    ...
    return "Both base and quote must contain a field specified with dot notation", None
  ...
  try:
    ...
    return "Required fields not found in resources", None
