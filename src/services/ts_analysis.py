...
async def ensure_df(
  ...
  if df is None:
    ...
    err, result = await loader.get_history(resources, fields, from_date, to_date, interval, quote_param, precision, "polars")
    if err:
...
async def add_metrics(
  ...
  def compute(resource: str, col: str, period: int) -> Tuple[dict, Union[str, None]]:
    try:
...
async def get_all(
  ...
  try:
    err, df = await get_volatility(resources, fields, from_date, to_date, interval, periods, quote, precision, "polars", df)
    if err:
      return err, None

    err, df = await get_trend(resources, fields, from_date, to_date, interval, periods, quote, precision, "polars", df)
    if err:
      return err, None

    err, df = await get_momentum(resources, fields, from_date, to_date, interval, periods, quote, precision, "polars", df)
    if err:
      return err, None
...
async def get_oprange(
  ...
  err, df = await ensure_df(resources, fields, from_date, to_date, "m5", quote, precision, None, 1000)
  if err:
    return err, pl.DataFrame()
  ...
  def compute_range_metrics(df: pl.DataFrame, col: str) -> dict:
    ...
    try:
      ...
      if (current is not None and min_val is not None and range_val is not None and range_val != 0):
  ...
  if range_metrics:
    ...
    metrics_row = {**{col: df[col].tail(1).item() if len(df[col]) > 0 else None for col in df.columns}, **range_metrics}
    result_df = pl.DataFrame([metrics_row])

  err_fmt, result = loader.format_table(result_df, from_format="polars", to_format=format)
  if err_fmt:
    return err_fmt, pl.DataFrame()
