...
class ConfigVersionManager:
  ...
  def _get_config_path(self) -> Path:
    ...
    config_path = (state.args.config if hasattr(state.args, "config")
                   and state.args.config else "chomp/ingesters.yml")
...
async def validate_config(
    config_content: str) -> ServiceResponse[Dict[str, Any]]:
...
async def get_config_history(
    limit: int = 10) -> ServiceResponse[List[Dict[str, Any]]]:
