...
Interval = Literal[
  "s1", "s2", "s5", "s10", "s15", "s20", "s30", # sub minute
...
def floor_date(date: Optional[datetime]=None, interval: Optional[str|int|float]=None) -> datetime:
  ...
  floored_epoch = epoch_sec - (epoch_sec % int(interval)) # only floor if not already at the floor
  floored_datetime = datetime.fromtimestamp(floored_epoch, date.tzinfo)
  return floored_datetime

def ceil_date(date: Optional[datetime]=None, interval: Optional[str|int|float]=None) -> datetime:
  date = date or now()
  if isinstance(interval, str):
    interval = interval_to_seconds(interval)
  if interval is None:
  ...
  return floored + timedelta(seconds=float(interval)) if floored != date else date # only shift if not already at the ceiling

def secs_to_ceil_date(date: Optional[datetime]=None, secs: Optional[int]=None, offset=0) -> int:
  date = date or now()
...
def shift_date(date: Optional[datetime]=None, timeframe: Interval="m1", backwards=False):
  date = date or now()
  return date + interval_to_delta(timeframe, backwards)

def fit_interval(from_date: datetime, to_date: Optional[datetime]=None, target_epochs=100) -> Interval:
...
def fit_date_params(from_date: Optional[datetime] = None, to_date: Optional[datetime] = None, interval: Optional[Interval] = None, target_epochs: Optional[int] = None) -> tuple[datetime, datetime, Interval, int]:
  ...
  if not target_epochs:
    if from_date and interval and to_date:
      target_epochs = int((to_date - from_date).total_seconds() // SEC_BY_TF[interval])
    else:
      target_epochs = 400
