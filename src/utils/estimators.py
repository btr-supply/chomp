...
def garman_klass(high: pl.Series,
                 low: pl.Series,
                 open: pl.Series,
                 close: pl.Series,
...
def rogers_satchell(high: pl.Series,
                    low: pl.Series,
                    open: pl.Series,
                    close: pl.Series,
                    period: int = 20) -> pl.Series:
...
def sma(data: Union[List, pl.Series],
        period: int = 20) -> Union[List[float], pl.Series]:
...
def rsi(data: Union[List, pl.Series],
...
def stochastic(close: pl.Series,
               high: pl.Series,
               low: pl.Series,
               period: int = 14) -> tuple[pl.Series, pl.Series]:
...
def zscore(s: pl.Series, period: int = 20) -> pl.Series:
  """Z-score of returns"""
  returns = roc(s)
  return (returns - returns.rolling_mean(window_size=period)
          ) / returns.rolling_std(window_size=period)
...
def adx(high: pl.Series,
  ...
  df = df.with_columns([
      pl.when(pl.col("up_move") > pl.col("down_move")).then(
          pl.col("up_move")).otherwise(0).clip(lower_bound=0).alias("pos_dm"),
      pl.when(pl.col("down_move") > pl.col("up_move")).then(
          pl.col("down_move")).otherwise(0).clip(
              lower_bound=0).alias("neg_dm"),
  ])
...
def ema(data: Union[List, pl.Series],
        period: int = 20) -> Union[List[float], pl.Series]:
...
# Bollinger Bands
def bollinger_bands(
    data: Union[List, pl.Series],
    period: int = 20,
    std_dev: float = 2
) -> Union[Tuple[List[float], List[float], List[float]], Tuple[
...
def volatility(returns: Union[List, pl.Series]) -> float:
  ...
  if isinstance(returns, list):
    ...
    result = s.std()
    return float(
        result) if result is not None else 0.0  # type: ignore[arg-type]
  else:
    result = returns.std()
    return float(
        result) if result is not None else 0.0  # type: ignore[arg-type]
...
def rolling_volatility(returns: Union[List, pl.Series],
...
# Prediction models
def linear_regression(x: Union[List, pl.Series],
                      y: Union[List, pl.Series]) -> Tuple[float, float]:
...
def moving_window(data: Union[List, pl.Series],
